...
const PLANE_DIVS_WIDTH = 21; // If this is even, it looks like there's a straight line down the center.
...
export function createSceneInfo(renderer) {
    ...
    sceneInfo.bloomPass.threshold = 0.2
    sceneInfo.bloomPass.strength = 1.0
...
function createTerrain(offset, sceneInfo) {
    ...
    var planeMesh = new THREE.Mesh(planeGeom, new THREE.MeshLambertMaterial({
        color: 0x0F7DB0,
    }))
...
function genTerrainVerts(plane, sceneInfo) {
    ...
    plane.geometry.computeVertexNormals(); // Costly
